...
export default function Restaurant_Menu({onSendData,onSubmitMenu3,menuList}){
    ...
    return(
        <>
            <div className="food-list">
                    <FoodSwiper onSubmitMenu2={onSubmitMenu} menuList={menuList}/>
                ...
                <div style={{display:iconOption?'block':'none'}}>
                    <ul className="popular-menu">
                        {menuList?menuList.map(item=>(
                            <li onClick={()=>menuClick(item.idx)}>
                                <div className="menu-text">
                                    <div className="menu-text-title">{item.name}</div>
                                    <div className="menu-text-info">100% 국내산 돼지고기, 양파, 춘장을 볶아 만든 기본에 충실한 짜장면</div>
                                    <div className="menu-text-price">{item.price.toLocaleString()}</div>
                                </div>
                                <div className="popular-menu-img"><img src={item.image} alt="" /></div>
                            </li>
